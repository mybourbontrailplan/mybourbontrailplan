...
- **A host can add one pin for their own property** via `?hostlat`/`?hostlng`/`?hostname`, embed mode only, generated by the optional section in `embed-bourbon-trail-map.html`. **The URL is the entire storage mechanism**: no database, no JSON file, nothing in the distillery dataset, and the pin never appears on our own map or another host's embed. Adding lodging to the canonical data would cost the map the neutrality that makes it worth embedding, and would invite every future host to ask for a listing. `hostname` is third-party text rendered in our origin, so it is `textContent` only; see `NOTES-internals.md` before touching it.
