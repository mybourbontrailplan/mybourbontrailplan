...
- **The SVG's `width`/`viewBox`/path/circle coordinates are per-banner and hard-coded** to that heading's rendered text width. `W` = the heading's rendered pixel width (measured at desktop, 22px Fraunces 700) **plus 10**; the path is `M5,10 Q {W/2},3 {W-5},8` and the right dot is at `cx={W-5}` (SVG attributes can't do math, so the numbers are pre-computed). Current values: Louisville `W=116`, Bardstown &amp; New Hope `W=272`, Frankfort `W=118`, Lexington `W=120`.
- **If a region banner heading's text changes, re-measure that heading and regenerate its trail-underline SVG width values** (`width`, `viewBox`, the `Q {W/2},3 {W-5},8` control/end points, and the end-dot `cx`). A stale width makes the arc under- or over-run the text. The other three banners are unaffected — only re-measure the one whose text changed.
