...
### Host pin (`?hostlat`, `?hostlng`, `?hostname`) - embed mode only
One optional marker for the property of the host who embedded the widget. It lives entirely in that host's iframe URL: **nothing is stored anywhere**, the pin never appears on the canonical map or on another host's embed, and lodging stays out of the distillery dataset. Generated by the optional section in `embed-bourbon-trail-map.html`.

- **`hostname` is attacker-controlled text rendered inside our own origin.** It is set with `textContent` on a DOM element passed to `bindPopup`. **Never build this popup as an HTML string**, the way the distillery popups directly above it are built. Verified with `<img src=x onerror=alert(1)>`: renders as literal text, one text node, no `<img>`, no execution.
- Validation, all silent on failure: strict numeric format before `parseFloat` (so `37abc` is rejected rather than read as 37), a Kentucky-and-margin box of lat 36.0-39.5 / lng -90.0 to -81.0, 5 decimal places, control and bidi codepoints scrubbed to spaces **before** the 60-character truncation. Anything malformed degrades to the plain map, never an error state.
- **The pin is deliberately not in `markers[]`**, which drives region filtering, the sidebar list, search and the "Showing N distilleries" count.
- **It needs its own Leaflet pane, not just `zIndexOffset`.** `zIndexOffset` only orders markers inside the marker pane (600), and the always-on distillery labels are in the tooltip pane (650), so labels paint over the pin. Measured at 53% of the pin covered with a host a few hundred feet from Willett. It uses pane `hostPin` at z-index 675, between tooltips and popups, with `pointerEvents:'auto'` so it stays clickable. The pane is created only when a pin exists.
- **Colour is navy fill + gold ring + white core, not brand gold.** `#D4A03C` is already the Bardstown pin colour and hosts cluster around Bardstown, so a gold pin would read as one more distillery.
- **`frameToIntendedView()` has to know about it.** That re-frame runs for 3 seconds after load to fix lazy-loaded iframes; without a host branch it snaps a host-centred widget back to statewide bounds exactly in the lazy case it exists to fix.
- Centring is a bounds fit of roughly 30 miles around the pin, not a fixed zoom, so it survives 520px and 820px frame heights. An explicit `?region=` or `?distillery=` wins; the pin still renders.
